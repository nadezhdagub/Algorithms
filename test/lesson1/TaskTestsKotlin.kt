...
class TaskTestsKotlin : AbstractTaskTests() {
    ...
    fun testMergeArrays() {
        ...
        val result2 = arrayOf(null, null, 1)
        mergeArrays(arrayOf(1, 2), result2)
        assertArrayEquals(arrayOf(1, 1, 2), result2)
        val empty = arrayOf<Int?>()
        mergeArrays(arrayOf<Int>(), empty)
        assertArrayEquals(arrayOf<Int?>(), empty)
        run {
